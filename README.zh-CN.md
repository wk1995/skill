...
| `pr-review-loop` | `pr-review-loop` 用于对 Pull Request 执行评审闭环：审查当前 head，把本轮确认的问题以 `[<平台>][<模型>]` 标注评论到 PR，随后修复、提交推送，再重新审查，直到某一轮不再发现确认问题为止。它只定义闭环本身——身份标注、轮次顺序、终止条件与汇报，刻意不定义「该怎么审查」。 | [README](skills/pr-review-loop/README.zh-CN.md) |
