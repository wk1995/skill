...
| `version-upgrade-policy` | `version-upgrade-policy` 用于定义、选择、校验和应用项目自身的发布版本，涵盖初始值、升级级别、自定义格式、发布依据、版本 CI 检查，以及写入 AGENTS.md 的长期规则。项目指令要求评估版本影响时，普通代码变更也会触发；仅提及版本号或执行已选定版本的发布时不触发。 | [README](skills/version-upgrade-policy/README.zh-CN.md) |
<!-- skills-catalog:end -->
