...
DEFAULT_SCHEMA = ROOT / "references" / "skill-relationships.schema.json"
...
def schema_statuses(schema_path: Path) -> set[str]:
    ...
    if "$ref" in schema:
        ...
        if not isinstance(reference, str) or ":" in reference or "#" in reference:
            raise ContractError("schema vocabulary reference must be a local file")
        schema = load_json(schema_path.parent / reference)
    try:
