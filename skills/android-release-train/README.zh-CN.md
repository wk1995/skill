...
在 OpenAI Codex 中可以用 `$android-release-train` 显式调用。在 ZCode 中，同样的请求会依据 `description` 与 `when_to_use` 元数据自动触发；在本仓库执行一次 `scripts/link-zcode-skill.sh` 即可链接到 `~/.zcode/skills/`。
