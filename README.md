...
| `version-upgrade-policy` | Define and apply project release-version rules, custom formats, initial values, changelog evidence, and CI checks. Add or update AGENTS.md rules that trigger proactive version-impact assessment after code changes. Use when version-policy work is requested or required by project instructions; exclude dependency/tool upgrades or build/publish execution alone. | [README](skills/version-upgrade-policy/README.md) |
<!-- skills-catalog:end -->
