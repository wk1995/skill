...
`SKILL.md` 是唯一必需入口。Skill 名称和版本以其 YAML frontmatter 为单一事实源，触发规则可在同一处按需补充：
...
- 扫描和解析 Skill。
- 校验目录名、frontmatter、版本、触发规则与扩展清单。
- 构建内存 registry。
- 安装、链接、升级、卸载和发布编排。
- 版本解析、依赖检查、摘要计算和信任策略。
...
`registry.json` 至少包含名称、版本、描述、触发规则、相对路径、是否含可执行扩展、支持的适配器、内容摘要和构建提交。生成物中的版本目录不可原地修改；新版本产生新的不可变目录。
